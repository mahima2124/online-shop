...
function Item() {
  ...
  const selectedProductId = selectedProducts[0].id;

  let index;
  const isProductInCart = () => {
    index = cartItems.findIndex((cartItem) => cartItem.id === selectedProductId);
  };
  isProductInCart();
        ...
        <div className="details" key={item.id}>
          <div className="big-img">
            ...
            <div className="box">
              ...
              {index === -1
                ? <button type="button" className="cart" onClick={() => handleAddToCart(item)}>Add To Cart</button>
                : <Link to="/cart"><button type="button" className="cart">Go To Cart</button></Link>}
            </div>
          </div>
        </div>
      ))}
